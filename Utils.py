...
def SubpixelConv2D(input_shape, scale=4):
    def subpixel_shape(input_shape):
        dims = [input_shape[0], input_shape[1] * scale, input_shape[2]
                * scale, int(input_shape[3] / (scale ** 2))]
...
def lr_images(images_real, downscale):
    ...
    for img in range(len(images_real)):
        images.append(imresize(images_real[img], [
                      images_real[img].shape[0]//downscale, images_real[img].shape[1]//downscale], interp='bicubic', mode=None))
...
def load_training_data(directory, ext, number_of_images, train_test_ratio):
    ...
    if len(files) < number_of_images:
        print("圖片檔案數量少於你指定的數量")
        print("請減少你的資料量至 %d" % len(files))
        sys.exit()
